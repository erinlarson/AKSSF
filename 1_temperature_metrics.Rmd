...
Add in the regions and thermal sensitivity from the model data frame, but note that TS is from 2011-2019, whereas the metrics dataset is all sites and years with complete summer time series.
...
metrics2 <- readRDS("output/metrics_2022-05-11.rds")
...
#get rid of deshka sites that overload analysis.
mets_lg %>% 
  distinct(SiteID, subset1) %>% 
  count(subset1)
...
mets11_scwd <- mets_lg %>%
  ...
  select(Region, SiteID, Year, metric_lab, value_sc) %>% 
  pivot_wider(names_from = metric_lab, values_from = value_sc)

summary(mets11_scwd)

mets11_dist <- dist(mets11_scwd %>% select(-SiteID, -Year, -Region))
...
temp_groups <- bind_cols(mets11_scwd %>% select(SiteID), grp4 = cutree(mets11_clust, k = 4), grp6 = cutree(mets11_clust, k = 6), grp3 = cutree(mets11_clust, k = 3))

temp_groups %>% count(grp6) %>% mutate(percent = n/sum(n))
...
bind_cols(mets11_scwd, group = cutree(mets11_clust, k = 6)) %>% 
  ...
  group_by(Region) %>% 
  mutate(percent = n/sum(n)) %>% 
  ggplot(aes(x = Region, fill = as.factor(group), y = percent)) +
  geom_col() +
  theme_bw() +
...
nbclust1 <- NbClust(data = mets11_scwd %>% select(-SiteID, -Year, -Region), min.nc = 2, max.nc = 10, method = "ward.D2", index = "all")
...
mets11_env <- mets11_scwd %>% 
  select(Region, SiteID, Year) %>% 
  mutate(Yearf = factor(Year))

pc1 <- prcomp(mets11_scwd %>% select(-Region, -SiteID, -Year)) 

pc2 <- princomp(mets11_scwd %>% select(-Region, -SiteID, -Year), cor = TRUE) 
...
#region ellipses
autoplot(pc1, data = mets11_env, colour = 'gray', labels = TRUE, loadings.label = TRUE, 
