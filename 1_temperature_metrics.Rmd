...
left_join(metrics %>% rename(Site = SiteID), mod_dat %>% select(Site, Year, TempSens, Region)) %>% 
  filter(is.na(TempSens)) %>% 
  count(Year) #mostly 2020


metrics2 <- left_join(metrics %>% rename(Site = SiteID), mod_dat %>% select(Site, Year, TempSens, Region)) 
...
Observations: (note to Dustin: I trust accs, cik (including deshka sites that start with deshka, moose, or kroto), usgs so skimmed those, focussed on other data providers...)
