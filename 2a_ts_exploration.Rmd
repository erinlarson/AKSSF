...
```{r}
dfa_ts %>% 
  ggplot(aes(TempSens_Air)) +
  geom_freqpoly(aes(color = Region)) +
  facet_wrap(~Year)

dfa_ts %>% 
  ggplot(aes(TempSens_Air.DayLen)) +
  geom_freqpoly(aes(color = Region)) +
  facet_wrap(~Year)
...
Use merge table to join on SiteIDs. One site in responses without spatial data - Egegik River, checking with Dustin on that one. fws_580223156504200 with data for 2015-2018.
...
cov.all <- cov.all %>% mutate(cov_id = tolower(SiteID),
...
dfa_ts %>% 
  ...
  select(-Region) %>% 
  left_join(cov.all %>% select(-SiteID)) %>% 
  filter(is.na(str_ord)) 

md %>% filter(Site == "fws_580223156504200")
...
mod_dat <- left_join(mod_dat %>% rename(Site = SiteID) %>% select(-site_lower, -X1, -cov_id, -region), precip) 
summary(mod_dat)
...
Look at sites with lots of negative TS as a check. original list of sites using just air temperature and no daylength is much different than model with both in it. For now, leave all sites. Note that some of the lake affected sites may be very difficult to model TS for since lake temperatures can change so quickly.
...
```
# STOPPED HERE 4/22/22. NOTE THAT I AM NOT USING SCRIPTS B AND D EITHER SO MAYBE DELETE THOSE OR NOTE ON TOP.

## Covariates 

Checking outliers and collinearity. Drop those with high correlation.

Check slopes first. A couple of outliers -- one for stream slope and two for minimum catchment slope. We checked the min catchment slopes and they are accurate, streams in very steep watersheds. One of those also had the high stream slope. Note that the mean, min, max statistics for slope and elevation by watershed are all highly correlated so just go with means for now and recheck using r and vif.
