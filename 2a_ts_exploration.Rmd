...
cov.all <- left_join(sites.df %>% select(SiteID, Region, ts_id, cat_ID_con, str_ord, str_slope, dist_coast_km),
                     cov.df)

cov.all <- left_join(cov.all, lcld.long)
summary(cov.all)

# saveRDS(cov.all %>% select(SiteID, ts_id), "data_preparation/spatial_ts_IDjoin.rds")
...
cov.all %>% 
  filter(Region == "Cook_Inlet") %>% 
  ggplot() +
  geom_freqpoly(aes(x = wtd_lcld_jd, after_stat(density))) +
  facet_wrap(~Year)
...
mod_dat <- dfa_ts %>% 
  ...
  left_join(cov.all %>% select(-SiteID, -Region), by = c("site_lower" = "ts_id", "Year" = "Year")) 
...
left_join(mod_dat, md %>% select(Site, Waterbody_name, SourceName)) %>% 
  ...
  group_by(Site, Waterbody_name, wtd_glacier_per) %>%
...
mod_dat %>% 
  mutate(snow_tc = (wtd_lcld_jd - 1.783443 * wtd_slope_MEAN)) %>% 
  summary(snow_tc)
...
10/13/22 Checking snow index and Tim's snow covariate.

```{r compare to TC data}
tc_cov <- read_csv("S:/Stream Temperature Data/Tim Cline UW/StreamGeomorphologyAndClimate.csv")
tc_site_merge <- read_csv("W:/Github/SWSHP-Bristol-Bay-Thermal-Diversity/output/UW_site_merge_rs.csv")

tc_cov <- left_join(tc_cov, tc_site_merge, by = c("Stream" = "tc"))

tc_cov %>% select(Stream, uw)

tc_compare <- left_join(tc_cov, mod_dat2 %>% filter(grepl("UW", Site)) %>% mutate(uw = gsub("UW_", "", Site))) 

left_join(tc_cov %>% filter(grepl("Teal", uw)), mod_dat2 %>% filter(grepl("UW", Site)) %>% mutate(uw = gsub("UW_", "", Site)) %>% filter(grepl("Teal", uw)))

tc_cov %>% filter(grepl("Teal", uw)) 

mod_dat2 %>% filter(grepl("Teal", Site)) 

saveRDS(tc_compare, "output/combined_bb_data.rds")

tc_compare %>% 
  filter(is.na(wtd_slope_MEAN))  
...
# Comparing 2022 versus 2023 covariates

Read in both data frames. Compare the final covariates used in the model across the two datasets:
stream slope, mean catchment elevation, mean catchment slope, mean watershed slope, percent of the watershed with a north aspect, watershed area, distance to the coast, percent of the watershed covered by glaciers, lakes, or wetlands; an index of spring snow cover; total summer precipitation; and region 

Dustin created new covariates using a new lake dataset, but noticed that many other covariates also changed and we're now wondering why. Did ESRI change their code?

```{r SiteIDs for spatial and response data}
...
#spatial sites
cov23 <- read_csv("data_preparation/final_data/AKSSF_sites_sj_merge_2023.csv")


anti_join(temp_dat %>% distinct(SiteID), cov23 %>% distinct(SiteID))
```




```{r read in old and new covariates}
cov22 <- readRDS("data_preparation/final_data/model_data2022-05-09.rds")
...
anti_join(cov22 %>% distinct(SiteID = Site), cov23 %>% distinct(SiteID = ts_id_dm))

cov_comp <- left_join(cov22 %>%
                        mutate(SiteID = tolower(Site)) %>% 
            distinct(SiteID,
                     str_slope,
                     cat_elev_MEAN,
                     cat_slope_MEAN,
                     ...
                     wtd_wet_per),
          cov23 %>% 
            distinct(SiteID = ts_id_dm,
                     str_slope,
                     cat_elev_MEAN,
                     cat_slope_MEAN,
                     wtd_slope_MEAN,
                     wtd_north_per,
                     # wtd_area_sqKM = wtd_area, #dustin needs to let me know which one to use
                     dist_coast_km,
                     wtd_glacier_per,
                     wtd_lake_per,
                     wtd_wet_per) %>% 
            rename_with( ~ paste0(.x, "_23"), -SiteID)
)

cov_comp %>% 
  filter(is.na(str_slope_23))
```


```{r compare covariates}

#stream slope
cov_comp %>% 
  ggplot(aes(x = str_slope, y = str_slope_23)) +
  geom_point() +
  geom_abline(aes(slope = 1, intercept = 0))

#watershed slope
cov_comp %>% 
  ggplot(aes(x = wtd_slope_MEAN, y = wtd_slope_MEAN_23)) +
  geom_point() +
  geom_abline(aes(slope = 1, intercept = 0))

#lake cover
cov_comp %>% 
  ggplot(aes(x = wtd_lake_per, y = wtd_lake_per_23)) +
  geom_point() +
  geom_abline(aes(slope = 1, intercept = 0))

#north aspect
cov_comp %>% 
  ggplot(aes(x = wtd_north_per, y = wtd_north_per_23)) +
  geom_point() +
  geom_abline(aes(slope = 1, intercept = 0))

#distance to coast
cov_comp %>% 
  ggplot(aes(x = dist_coast_km, y = dist_coast_km_23)) +
  geom_point() +
  geom_abline(aes(slope = 1, intercept = 0))

#glacier cover
cov_comp %>% 
  ggplot(aes(x = wtd_glacier_per, y = wtd_glacier_per_23)) +
  geom_point() +
  geom_abline(aes(slope = 1, intercept = 0))
```

AWC species and life stage information. (Dustin will add this information to the variables names google sheet so we know what each means.)

```{r AWC columns for sites}

#sp ls is species and life stage information
cov23$sp_ls

#sa sp hab has 1, 2 and NA
# 1 = 
# 2 = 
# NA = 
cov23$sa_sp_hab %>% unique()
