...
mod_dat %>% 
  filter(TempSens < 0) %>% 
  select(Site, Year, TempSens, wtd_glacier_per, wtd_lake_per, wtd_north_per, wtd_lcld_jd) %>% 
  arrange(TempSens)


md %>% filter(SiteID == "572656154082400")
summary(mod_dat)
```
...
what about other sites in Luca's dataset?

* middle arm eyak - remove?
* middle fork 18 mile
* 24.9 mile creek
...
mod_dat %>%  
  filter(grepl("USFS|Martin", Site)) %>% distinct(Site)
...
mod_dat2 %>% 
  select(log_slope, log_cat_elev, log_cat_slope, wtd_north_per,
         ...
         snow_ind, asrt_glac, asrt_lake, asrt_wet, glac_10) %>% 
  pairs(upper.panel = panel.cor, diag.panel = panel.hist)
...
mod_dat2 %>% 
  select(log_slope, log_cat_elev, log_cat_slope, wtd_north_per,
         wtd_elev_MEAN, wtd_slope_MEAN, log_area, dist_coast_km,
         snow_ind, asrt_glac, asrt_lake, asrt_wet) %>% 
  apply(., 2, function(x) skewness(x))

mod_dat2 %>% 
  select(wtd_glacier_per, wtd_lake_per, wtd_wet_per, logit_glac, logit_lake, snow_ind,
         asrt_glac, asrt_lake, asrt_wet) %>% 
  pairs(upper.panel = panel.cor, diag.panel = panel.hist)
...
lm1 <- lm(TempSens ~ log_slope + log_cat_elev + log_cat_slope + wtd_north_per +
         wtd_elev_MEAN + wtd_slope_MEAN + log_area + dist_coast_km + 
         asrt_wet + asrt_glac + asrt_lake + glac_10 + snow_ind + summer_precip, data = mod_dat2)

vif(lm1)
```

Remove mean watershed elevation now all vif < 3 or close to it. Wtd slop is highest at 3.17 and the only one above 3.
...
#saving new version with additional covariate transformations 2/2/22
