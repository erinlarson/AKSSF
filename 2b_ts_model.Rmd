...
lme1Ar <- lme(TempSens ~ log_slope + cat_elev_MEAN + cat_slope_MEAN + wtd_north_per +
              ...
              data = train, random = ~1 | Region/Site, method = "REML")
...
lme1Br <- lme(TempSens ~ log_slope + cat_elev_MEAN + cat_slope_MEAN + wtd_north_per +
                ...
                wtd_north_per*snow_ind + wtd_north_per*wtd_slope_MEAN + wtd_north_per*summer_precip +
                summer_precip*wtd_slope_MEAN + wtd_slope_MEAN*snow_ind, 
              data = train, random = ~1 | Site, method = "REML")
...
lme1Cr <- lme(TempSens ~ log_slope + cat_elev_MEAN + cat_slope_MEAN + wtd_north_per +
                ...
                wtd_north_per*snow_ind + wtd_north_per*wtd_slope_MEAN + wtd_north_per*summer_precip +
                summer_precip*wtd_slope_MEAN + wtd_slope_MEAN*snow_ind, 
              data = train, random = ~1 | Region, method = "REML")

lme1Dr <- gls(TempSens ~ log_slope + cat_elev_MEAN + cat_slope_MEAN + wtd_north_per +
                wtd_slope_MEAN + log_area + dist_coast_km + 
                wtd_wet_per + logit_glac + logit_lake + snow_ind +summer_precip +
                wtd_north_per*snow_ind + wtd_north_per*wtd_slope_MEAN + wtd_north_per*summer_precip +
                summer_precip*wtd_slope_MEAN + wtd_slope_MEAN*snow_ind, 
...
AIC(lme1Ar, lme1Br, lme1Cr, lme1Dr)
```
...
lme1m <- lme(TempSens ~ log_slope + cat_elev_MEAN + cat_slope_MEAN + wtd_north_per +
                ...
                wtd_north_per*snow_ind + wtd_north_per*wtd_slope_MEAN + wtd_north_per*summer_precip +
                summer_precip*wtd_slope_MEAN + wtd_slope_MEAN*snow_ind, 
             data = train, random = ~1 | Region/Site, method = "ML")
...
lme1m_nosite <- lme(TempSens ~ log_slope + cat_elev_MEAN + cat_slope_MEAN + wtd_north_per +
                ...
                wtd_north_per*snow_ind + wtd_north_per*wtd_slope_MEAN + wtd_north_per*summer_precip +
                summer_precip*wtd_slope_MEAN + wtd_slope_MEAN*snow_ind, 
             data = train, random = ~1 | Region, method = "ML")
...
lme1m <- lme(TempSens ~ log_slope + cat_elev_MEAN + cat_slope_MEAN + wtd_north_per +
                ...
                wtd_north_per*snow_ind + wtd_north_per*wtd_slope_MEAN + wtd_north_per*summer_precip +
                summer_precip*wtd_slope_MEAN + wtd_slope_MEAN*snow_ind, 
             data = mod_dat, random = ~1 | Region/Site, method = "ML")
...
Look at dredge results and use subset for model-selection. Test using cross validation. Set up groups by years (2 years at a time, n = 10) and regions (n = 5).
...
I tried using region for a first go, but then I can't predict using that using the null model, so try other grouping based in HUCs that is a little larger (8 groups).
...
Compare spatial and temporal xvalidation metrics:

* global model
* drop covariates and interactions in group with lowest variable important - 7
* drop next group - 7
* drop final group of 2 only keeping top 4 predictors
...
lme_dr7 <- formula(TempSens ~ cat_elev_MEAN + wtd_north_per +
                wtd_slope_MEAN + log_area + dist_coast_km + 
                logit_glac + logit_lake + snow_ind +summer_precip +
                summer_precip*wtd_slope_MEAN)

lme_dr11 <- formula(TempSens ~ wtd_slope_MEAN + logit_glac + logit_lake + snow_ind +summer_precip +
                summer_precip*wtd_slope_MEAN)

lme_dr13 <- formula(TempSens ~ wtd_slope_MEAN + logit_lake + summer_precip +
                summer_precip*wtd_slope_MEAN)
  
```

Temporal xvalidation. Want to create a data frame with the model, the set of years using for training, the set of years using for testing, the internal rmse, and the external rmse. 

```{r temporal cross-validation}

xval_results <- data.frame()
...
for(i in 1:5) {
  ...
  #global model
  lmegl <- lme(lme_all, data = train, random = ~1 | Region/Site, method = "REML")    
  lme2 <- lme(lme_dr7, data = train, random = ~1 | Region/Site, method = "REML")    
  lme3 <- lme(lme_dr11, data = train, random = ~1 | Region/Site, method = "REML")    
  lme4 <- lme(lme_dr13, data = train, random = ~1 | Region/Site, method = "REML")    
  lme5 <- lme(TempSens~1, data = train, random = ~1 | Region/Site, method = "REML")    

  xval_results <- bind_rows(xval_results,
    bind_cols(test %>% select(Site, Year, TempSens), 
            global = predict(lmegl, newdata = test, level = 1),
            lme_10vars = predict(lme2, newdata = test, level = 1),
            lme_6vars = predict(lme3, newdata = test, level = 1),
            lme_4vars = predict(lme4, newdata = test, level = 1),
            lme_null = predict(lme5, newdata = test, level = 1)) %>% 
    pivot_longer(names_to = "model", values_to = "preds", cols = global:lme_null) %>% 
    mutate(sqerror = (preds - TempSens)^2,
           abserror = abs(preds - TempSens)) %>%
    group_by(model) %>% 
    summarize(rmse = sqrt(mean(sqerror)),
              mae = mean(abserror),
              obs_pred = cor(preds, TempSens),
              obs_pred_rank = cor(preds, TempSens, method = "spearman")) %>%
...
for(i in 1:8) {
  sp_xval <- mod_dat %>% distinct(sp_xval) %>% slice(i) 
  train <- mod_dat %>% 
    anti_join(sp_xval)
  test <- mod_dat %>% 
    right_join(sp_xval)
  ...
  #global model
  lmegl <- lme(lme_all, data = train, random = ~1 | Site, method = "REML")    
  lme2 <- lme(lme_dr7, data = train, random = ~1 | Site, method = "REML")    
  lme3 <- lme(lme_dr11, data = train, random = ~1 | Site, method = "REML")    
  lme4 <- lme(lme_dr13, data = train, random = ~1 | Site, method = "REML")    
  lme5 <- lme(TempSens~1, data = train, random = ~1 | Site, method = "REML")    

  xval_results <- 
    bind_rows(xval_results, 
              bind_cols(test %>% select(Site, Year, TempSens), 
                        global = predict(lmegl, newdata = test, level = 0),
                        lme_10vars = predict(lme2, newdata = test, level = 0),
                        lme_6vars = predict(lme3, newdata = test, level = 0),
                ...
                mutate(xval_type = "spatial",
                       xval_group = sp_xval %>% pull(sp_xval))
    )
}
...
```{r plot xval results}
xval_results %>% 
  mutate(modelf = factor(model, levels = c("global", "lme_10vars", "lme_6vars", "lme_4vars", "lme_null"))) %>% 
  ggplot(aes(x = modelf, y = rmse, color = xval_type)) +
  stat_summary(fun = "mean", geom = "point") +
  stat_summary(fun.data = "mean_se", geom = "errorbar", width = 0.2) +
  labs(x = "RMSE on Validation Data", y = "Model", color = "Cross-validation Type") +
  theme_bw() +
...
#copper river very hard to predict when held out - null model performs best!
xval_results %>% 
  arrange(desc(rmse))
```

Look at model with just four predictors.

```{r}
lme4 <- lme(lme_dr13, data = mod_dat, random = ~1 | Region/Site, method = "REML")
summary(lme4)

lme4$coefficients$random$Region
```
