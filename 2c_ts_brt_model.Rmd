...
pd_precip_lake <- pdp::partial(brt_step, pred.var = int_vars, 
              n.trees = 5200, train = brt_in, plot = TRUE, chull = TRUE, rug = TRUE)
pd_precip_lake

int_vars <- c("wtd_lcld_jd", "wtd_slope_MEAN")
pd_snow_slope <- pdp::partial(brt_step, pred.var = int_vars, 
              n.trees = 5200, train = brt_in, plot = TRUE, chull = TRUE, rug = TRUE)
pd_snow_slope
...
p1 <- brt_step$contributions %>% 
  ...
  #                              TRUE ~ var)) %>% 
  ggplot(aes(x = rel.inf, y = fct_reorder(var, rel.inf))) +
  geom_col() +
...
brt_out %>% 
  ...
  ggplot(aes(x = cv.preds, y = TempSens_Air)) +
  geom_point() +
...
brt_out %>% 
  mutate(sqerror = (TempSens_Air - cv.preds)^2) %>% 
...
Bring in hu12 outlet covariates and snowpack for all years. Can focus results on 2012 (high) and 2015 (low) years. Need to rename all covariates in these files so they match the names in the model, so prediction will work.
...
```{r TS predictions}
hu12.df <- hu12.df %>% 
  mutate(preds = predict(brt_step, newdata = hu12.df)) 

summary(hu12.df) 

hu12.df %>% 
  ggplot(aes(x = preds, color = Year)) +
  geom_freqpoly() +
  facet_wrap(~Year)

#this is a totally different result than Tim had where
# there were larger diffs in high sloped watersheds
hu12.df %>% 
  # filter(dist_coast_km > 50) %>% 
  ...
  ggplot(aes(x = wtd_slope_MEAN, y = snow_diff, size = dist_coast_km)) +
  geom_point()
...
hu12.df %>% 
  filter(Year %in% c(2012, 2015)) %>%
  select(HUC12, Year, preds) %>% 
  pivot_wider(names_from = Year, values_from = preds) %>% 
  mutate(ts_diff = `2012` - `2015`) %>% 
  ggplot() +
  geom_freqpoly(aes(x = ts_diff))

bind_rows(hu12.df %>% 
            group_by(HUC12) %>% 
            slice_min(preds, with_ties = FALSE) %>% 
            mutate(snow_year = "min"),
          hu12.df %>% 
            group_by(HUC12) %>% 
            slice_max(preds, with_ties = FALSE) %>% 
            mutate(snow_year = "max")) %>% 
  select(HUC12, snow_year, preds) %>%
  pivot_wider(names_from = snow_year, values_from = preds) %>% 
  mutate(ts_diff = max - min) %>% 
  ggplot() +
  geom_freqpoly(aes(x = ts_diff))
...
hu12 <- left_join(hu12, hu12.df %>% 
                    filter(Year %in% c(2012, 2015)) %>% 
                    select(HUC12, Year, preds, Region)) %>% 
  filter(!is.na(preds))

ggplot() +
  geom_sf(data = hu12, aes(fill = preds), color = NA) +
  facet_wrap(~Year)

hu12_2 <- left_join(hu12, bind_rows(hu12.df %>% 
  group_by(HUC12) %>% 
  slice_min(preds) %>% 
    mutate(snow_year = "min"),
  hu12.df %>% 
  ...
  slice_max(preds) %>% 
    mutate(snow_year = "max")) %>% 
    select(HUC12, snow_year, Year, preds, Region)) %>% 
  filter(!is.na(preds))

ggplot() +
  geom_sf(data = hu12_2, aes(fill = preds), color = NA) +
  facet_wrap(~snow_year)
  
```

How do TS vary for different salmon species by low and high snow? 

```{r}
hu12.snow <- bind_rows(hu12.df %>% 
            group_by(HUC12) %>% 
            slice_min(snow_tc, with_ties = FALSE) %>% 
            mutate(snow_year = "min"),
          hu12.df %>% 
            group_by(HUC12) %>% 
            slice_max(snow_tc, with_ties = FALSE) %>% 
            mutate(snow_year = "max")) %>% 
  ungroup()
...
hu12.snow %>% 
  ...
  filter(value == 1) %>% 
  ggplot() +
  geom_freqpoly(aes(x = preds, color = species)) +
  facet_grid(rows = vars(Region), cols = vars(snow_year))
