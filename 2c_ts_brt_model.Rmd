...
valconfine<-read.csv("data_preparation/sensitivity_drivers/AKSSF_ValleyConfinement_Metrics_Final_20240314.csv")
valconfine_only<-subset(valconfine, select=c("cat_ID_con", "cat_vb_mean_width_m"))
...
dat <- merge(dat, valconfine_only, by="cat_ID_con")
dat$log_vb_mean_width_m<-log(dat$cat_vb_mean_width_m)
  
head(dat)
cor(dat$wtd_slope_MEAN, dat$wtd_wet_per)
cor(dat$wtd_slope_MEAN, dat$wtd_elev_MEAN)

```

Explore range for stream slope and watershed area. We may not need to QA all the coastal HUC outlet catchments across the study area if they are steep small streams not well represented in the training dataset/model.

```{r}
dat %>% 
  ggplot(aes(x = str_slope*100, y = log(wtd_area_sqKM))) +
  geom_point() +
  geom_vline(aes(xintercept = 4, color = "strms")) +
  geom_hline(aes(yintercept = 0, color = "wtds")) +
  scale_color_manual(values = c("blue", "green"), 
                     labels = c("strms" = "Stream gradient > 5%",
                                "wtds" = bquote("Watersheds < 1 km"^2))) +
  theme(legend.title = element_blank()) +
  labs(x = "Stream Gradient (%)", y = bquote("Watershed Area ("~km^2~")"))

ggsave("output/gradient by watershed area.jpeg", units = "in", width = 9, height = 7)

dat %>% 
  distinct(Site, str_slope) %>% 
  ggplot(aes(x = str_slope*100)) +
  geom_histogram()

dat %>% 
  distinct(Site, str_slope) %>% 
  count(str_slope*100 > 4)

dat %>% 
  distinct(Site, wtd_area_sqKM) %>% 
  ggplot(aes(x = (wtd_area_sqKM))) +
  geom_histogram()

dat %>% 
  distinct(Site, wtd_area_sqKM) %>% 
  count(log(wtd_area_sqKM) < 2)
...
Transformed covariates: log_slope, log_cat_elev, log_cat_slope, wtd_north_per, wtd_slope_MEAN, log_area, dist_coast_km, asrt_glac, asrt_lake, asrt_wet, snow_ind, summer_precip, Region, log_vb_mean_width_m
...
brt_in <- dat %>% 
  ...
  dplyr::select(TempSens_Air, str_slope, cat_elev_MEAN, cat_slope_MEAN, 
                             ...
                             wtd_glacier_per, wtd_lake_per, wtd_wet_per, snow_tc, summer_precip, Region, cat_vb_mean_width_m)
...
Save a copy of the final TS along with 14 covariates for Dustin to add to the mapper. Include lat/long from md file so he can plot.

```{r save copy of TS and covariates for zenodo}
...
dat %>% 
  mutate(Region = as.factor(Region)) %>%
  complete(Site, Year) %>% 
  left_join(md %>% select(Site, Latitude, Longitude)) %>% 
  as.data.frame() %>% 
  dplyr::select(SiteID = Site, Year, TempSens_Air.DayLen, str_slope, 
                cat_elev_MEAN, cat_slope_MEAN, wtd_north_per, 
                wtd_slope_MEAN, wtd_area_sqKM, #dist_coast_km,
                wtd_glacier_per, wtd_lake_per, wtd_wet_per, 
                wtd_lcld_jd, summer_precip, Region, cat_vb_mean_width_m, Latitude, Longitude) %>% 
  write_csv(., file = "data_preparation/final_data/zenodo1/tempSitesSens.csv")
...
training data correlation = 0.937 
cv correlation =  0.839 ; se = 0.012 
...
brt_step <- readRDS("output/brt_dfa1tr_air2024-04-11.rds")
...
training data correlation = 0.902 
cv correlation =  0.719 ; se = 0.016 
...
brt_step_dl <- readRDS("output/brt_dfa1tr_AirDL2024-04-11.rds")
```
...
mean total deviance = 0.028 
mean residual deviance = 0.005 
 
estimated cv deviance = 0.014 ; se = 0.001 
 
training data correlation = 0.905 
...
brt_step_dl2 <- readRDS("output/brt_dfa1tr_AirDL_simp2024-04-11.rds")

```


```{r}
gbm.plot(brt_step_dl2, smooth = TRUE, n.plots = 14) #plots of first order relationships

brt_int <- gbm.interactions(brt_step_dl2) #identifies interactions
summary(brt_step_dl2)
...
#EL - This keeps causing R to abort so not re-running (as of April 5, 2024)
...
#Commenting out becauser this includes data that was on Becky's local drive and is just an example
...
p1 <- brt_step_dl2$contributions %>% 
  mutate(var_names = case_when(var == "wtd_slope_MEAN" ~ "Watershed slope",
                               ...
                               var == "str_slope" ~ "Stream Slope",
                               var == "cat_slope_MEAN" ~ "Mean Catchment Slope",
                               TRUE ~ var)) %>%
...
brt_out %>% 
  ...
  stat_regline_equation(label.x = -0.15, label.y = 0.7,
                        aes(label = ..eq.label..)) +
  stat_regline_equation(label.x = -0.15, label.y = 0.8,
                        aes(label = ..adj.rr.label..)) +
  theme_bw() +
  labs(y = "Observed Thermal Sensitivity",x = "Predicted Thermal Sensitivity") +
  theme(text = element_text(size = 14))

ggsave("output/brt OP single plot.jpeg", width = 6, height = 4)
```
...
hu12.cov <- read_csv("data_preparation/sensitivity_drivers/AKSSF_awcHuc12s_CovariatesWCONF_20240314.csv")
...
#dist_catch<-read_csv("data_preparation/sensitivity_drivers/AKSSF_dist_catch_coast_km.csv")
dput(names(brt_in)) #getting correct variable names from model



hu12.cov <- merge(hu12.cov, valconfine_only, by="cat_ID_con")
#hu12.cov <- merge(hu12.cov, dist_catch, by="cat_ID_con")
#hu12.spls <- merge(hu12.spls, dist_catch, by="cat_ID_con")
...
hu12.cov <- hu12.cov %>% 
  select(cat_ID_con, cat_elev_MEAN = AwcHuc12_cat_elev_MEAN,
         ...
         cat_vb_mean_width_m = cat_vb_mean_width_m.x,
         #dist_catch_coast_km = dist_catch_coast_km,
         AwcHuc12_wtd_lcld_mn_2011:AwcHuc12_wtd_lcld_mn_2019) %>% 
  pivot_longer(cols = AwcHuc12_wtd_lcld_mn_2011:AwcHuc12_wtd_lcld_mn_2019, names_to = "name", values_to = "wtd_lcld_jd") %>% 
...
hu12.spls <- hu12.spls %>% 
  select(cat_ID_con, HUC12, sp_ls#,
        # dist_coast_km = dist_catch_coast_km
        )
...
hu12_dm <- read_csv("data_preparation/HUC12_cat_pts_daymet_2011-19_UPDATE.csv") %>% 
  as_tibble()
...
hu12.df <- left_join(hu12.df, hu12_prp, by = join_by(cat_ID_con, Year)) %>% 
...
Check some precip values against the data extracted for the model - looks fine.

```{r}
dat %>% 
  filter(grepl("Moose 2 Upstream", Site))
#17 = 287, 18 = 380, 19 = 86

hu12.df %>% filter(HUC12 == "190205050702", Year %in% c(2017, 2018, 2019))
#17 = 288, 18 = 345, 19 = 92


dat %>% 
  filter(grepl("Jackpot", Site))
#14 = 1243, 15 = 628, 16 = 874, 17 = 886 

hu12.df %>% filter(HUC12 == "190202012501", Year %in% c(2014,  2015, 2016, 2017))
#14 = 1211, 15 = 613, 16 = 844, 17 = 845 
...
```{r}
saveRDS(hu12.df, "output/hu12_covars.rds")

hu12.df <- readRDS("output/hu12_covars.rds")
```


Comparing snowpack across hucs from 2012 to 2015 versus high versus low snow across all years.
We have a lot more variation in changes in snow from 2012 to 2015 than in the BB dataset. 
...
```{r TS predictions, eval = FALSE}
hu12.df <- hu12.df %>% 
  mutate(preds_air = predict(brt_step, newdata = hu12.df),
         preds_airDL = predict(brt_step_dl2, newdata = hu12.df)) 
...
Data frames of predicted TS differences comparing 2012/2015 and the highest and lowest snowpack years from 2011-2019.

```{r}
hu12.df <- hu12.df %>% 
  mutate(preds_airDL = predict(brt_step_dl2, newdata = hu12.df)) 
...
Exploring patterns in prediction for snow and precipitation - similar to interaction plot from model.

```{r}
#select a site of high slope and see if there is a difference.
hu12.df %>% 
  filter(HUC12 == "190202030404") %>% 
  ggplot(aes(size = snow_tc, y = preds_airDL, x = summer_precip)) +
  geom_point()

hu12.df %>% 
  filter(HUC12 == "190202030404") %>% 
  ggplot(aes(y = wtd_slope_MEAN, color = preds_airDL, x = summer_precip)) +
  geom_point() +
  scale_color_distiller(type = "div")

hu12.df %>% 
   filter(HUC12 == "190202030404") %>% 
  ggplot(aes(y = wtd_slope_MEAN, color = preds_airDL, x = snow_tc)) +
  geom_point() +
  scale_color_distiller(type = "div")
```
...
```{r TS maps low and hi snow}
hu12 <- st_read("S:\\Shared\\Aquatic\\AKSSF\\AKSSF_Hydrography.gdb", layer = "AKSSF_awcHuc12s_20240408")

hu12_snow <- bind_rows(hu12.df %>% 
                         group_by(HUC12) %>% 
                       ...
                       hu12.df %>% 
                         ...
                         mutate(snow_tc = "max")) %>% 
  select(HUC12, snow_tc, Year, preds_airDL, Region)


hu12_snow_sf <- left_join(hu12, hu12_snow) %>% 
  filter(!is.na(preds_airDL)) 

ggplot() +
  geom_sf(data = hu12_snow_sf, aes(fill = preds_airDL), color = NA) +
  facet_wrap(~snow_tc)
```

This confirms map of differences below, higher TS under low precipitation, which indicates that in rainier summers, there is higher flow and lower TS. But, this doesn't look right when we look at the interaction between precip and slope -- TS tends to increase under higher precip.

```{r TS maps low and high precip}

hu12_precip <- bind_rows(hu12.df %>% 
                           group_by(HUC12) %>% 
...
hu12_precip_sf <- left_join(hu12, hu12_precip) %>%  
  filter(!is.na(preds_airDL)) 

ggplot() +
  geom_sf(data = hu12_precip_sf, aes(fill = preds_airDL), color = NA) +
  facet_wrap(~precip_year) +
  scale_fill_continuous(type = "viridis")
```

Combined plot of change for both snow and precip.



```{r TS map for both snow and precip scenarios}


hu12_scens <- bind_rows(hu12_precip %>% 
                          rename(type = precip_year) %>% 
                          mutate(scenario = "Precipitation"),
                        hu12_snow %>% 
                          rename(type = snow_tc) %>% 
                          mutate(scenario = "Snowpack")) %>% 
  mutate(type = case_when(type == "min" ~ "Low",
                          type == "max" ~ "High"))


hu12_scens %>% arrange(HUC12)

hu12_scens_sp <- left_join(hu12, hu12_scens) %>% 
  filter(!is.na(preds_airDL))

ggplot() +
  geom_sf(data = hu12_scens_sp, aes(fill = preds_airDL), color=NA) +
  facet_grid(vars(type), vars(scenario)) +
  scale_fill_continuous(type = "viridis") +
  theme_bw() +
  labs(fill = expression(paste(tau)))

ggsave("output/mapped tau under 4 scens.jpeg", width = 6.5, height = 6)

```



```{r figure of mean tau for regions under different scenarios}

hu12_scens %>%
  st_drop_geometry() %>%
  mutate(Region = gsub("_", " ", Region)) %>% 
  group_by(Region, scenario, type) %>% 
  summarize(mean = mean(preds_airDL),
            sd = sd(preds_airDL)) %>% 
  ggplot(aes(x = Region, y = mean, color = type)) +
  geom_point(position = position_dodge(width = 0.5)) +
  geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd), width = 0.2, position = position_dodge(width = 0.5)) +
  facet_wrap(~scenario) +
  labs(y = expression(paste("mean ", tau)), color = "Scenario") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 75, hjust = 1), text = element_text(size = 14))

ggsave("output/mean tau under 4 scens.jpeg", width = 6, height = 4)
       
```


```{r TS maps change in TS as snow and precip increase}
...
ggplot() +
  ...
  labs(fill = expression(paste(Delta, tau)),
       title = expression(paste("Change in ", tau, " when precipitation or snowpack increases")))
  
```

Create a data frame with scenarios for snow and precipitation for each salmon stream. Add in species and life stage information as new columns and save a csv for Dustin in output folder so he can add to the web mapper.

```{r save prediction scenarios for web map}
hu12_spls <- left_join(hu12.df %>% distinct(HUC12, sp_ls), hu12_scens) %>%
...
write_csv(hu12_spls, paste0("output/scenarios_byhu12_spls_", Sys.Date(), ".csv"))
  
```

```{r tau scenarios for zenodo}
hu12_spls <- left_join(hu12_scens %>% 
                         mutate(Scenario = paste(type, scenario, sep = " ")) %>% 
                         select(Region, HUC12, Year, Scenario, TempSens = preds_airDL), 
                       hu12.df %>% select(HUC12, sp_ls, #dist_coast_km,
                                                      wtd_area_sqKM:cat_vb_mean_width_m, 
                                                      wtd_lcld_jd, Year, 
                                                      summer_precip)) %>%
  mutate(Chinook = case_when(grepl("K_", sp_ls) ~ 1,
                             TRUE ~ 0),
...
write_csv(hu12_spls, "data_preparation/final_data/zenodo1/tempScensSens.csv")
  
```



```{r tau figure by species and life stage}

#figure showing count of high or low snow by year for all huc12 outlets.
hu12_snow %>% 
  group_by(snow_tc, Region) %>% 
  count(Year) %>% 
  ggplot(aes(x = Year, y = n, color = snow_tc)) +
  geom_point() + 
...
hu12_spls %>% 
  pivot_longer(cols = Chinook:Rearing, names_to = "species") %>%
  mutate(species = factor(species, levels = c("Chinook", "Coho", "Sockeye", "Chum", "Pink", "Spawning", "Rearing"))) %>% 
  filter(value == 1) %>% 
  group_by(species, scenario, type) %>% 
  summarize(mean_ts = round(mean(preds_airDL), 2),
            sd_ts = round(sd(preds_airDL), 2)) %>% 
  ggplot(aes(x = species, y = mean_ts, color = type)) +
  geom_point(position = position_dodge(width = 0.5)) +
  geom_errorbar(aes(ymin = mean_ts - sd_ts, ymax = mean_ts + sd_ts), width = 0.3,
...
ggsave("output/mean tau by scens for species and life stages.jpeg", width = 6, height = 4)
...
# Kenai and Copper predictions

Select the lowermost huc12 outlet for each river basin, get it's watershed, intersect with all huc12 salmon outlets to get HUC12 for each watershed. Alternatively, do an intersection with huc12 outlet points to get list of HUC12s and then use that to filter the HUC12 layer and predictions.

Kenai River outlet = 190203021809 (note this watershed is incorrect, it grabbed a small trib to the lower kenai to the north)
190203021806 (one upstream)
Copper River outlet = 190201041203

```{r kenai and copper spatial data}

#watersheds

wtds <- st_read("T:\\Aquatic\\AKSSF\\AKSSF_Hydrography.gdb", 
                layer = "AKSSF_awcHuc12_Watersheds_Merge")

hu12pts <- st_read("T:\\Aquatic\\AKSSF\\AKSSF_Hydrography.gdb", 
                layer = "AKSSF_awcHuc12_outlet_cats_points")

wtds2 <- wtds %>% filter(HUC12 %in% c("190203021806", "190201041203")) %>% 
  select(region, HUC12, cat_ID_con, NHDPlusID)


#spatial intersect of watersheds with huc12


hu12pts %>% 
  mutate(intersection = as.integer(st_intersects(geometry, wtds2)),
         area = if_else(is.na(intersection), '', wtds2$region[intersection]))

ken_cop_huc12 <- st_intersects(wtds2, hu12pts)

ken_cop_huc12 <- hu12pts[ken_cop_huc12 %>% unlist(),]$HUC12

ken_strms <- st_read(dsn = "W:\\GIS\\AKSSF Southcentral\\AKSSF Southcentral.gdb", 
                     layer = "kenai_named_streams")

cop_strms <- st_read(dsn = "W:\\GIS\\AKSSF Southcentral\\AKSSF Southcentral.gdb", 
                     layer = "copper_named_streams")

st_crs(ken_strms) == st_crs(wtds2)
```


```{r kenai scenarios}
ggplot() +
  geom_sf(data = hu12_scens_sp %>% filter(HUC12 %in% ken_cop_huc12, region == "Cook_Inlet"), 
                                          aes(fill = preds_airDL), color = NA) +
  geom_sf(data = wtds2 %>% filter(region == "Cook_Inlet"), fill = NA) + 
  geom_sf(data = ken_strms, color = "gray") + 
  facet_grid(vars(type), vars(scenario)) +
  scale_fill_continuous(type = "viridis") +
  theme_bw() +
  labs(fill = expression(paste(tau)))
...
```{r copper scenarios}
ggplot() +
  geom_sf(data = hu12_scens_sp %>% filter(HUC12 %in% ken_cop_huc12, region == "Copper_River"), 
                                          aes(fill = preds_airDL), color = NA) +
  geom_sf(data = wtds2 %>% filter(region == "Copper_River"), fill = NA) + 
  geom_sf(data = cop_strms, color = "gray") + 
  facet_grid(vars(type), vars(scenario)) +
  scale_fill_continuous(type = "viridis") +
  theme_bw() +
  labs(fill = expression(paste(tau)))

ggsave("output/copper scenarios.jpeg", width = 8, height = 7)
