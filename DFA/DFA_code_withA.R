...
dfa_model_a <- "// Dynamic Factor Analysis for multivariate time series
#include <TMB.hpp>
...
Type objective_function<Type>::operator() ()
{
  ...
  write(dfa_model_a, file = "dfa1tmb_a.cpp")
  compile("dfa1tmb_a.cpp")
  dyn.load(dynlib("dfa1tmb_a"))
  ...
  runDFA_withA<-function(obs,NumStates=1,ErrStruc='DE',EstCovar=FALSE,Covars=NULL,indivCovar=FALSE,Dmat=NULL,Dfac=NULL,Rfac=NULL,logsdObs=NULL,logsdObsFac=NULL,cholCorr=NULL,cholFac=NULL,EstSE=FALSE){ 
    ##
    ...
    parameters <- list(
      ...
      D=Dmat,
      A=matrix(0,nrow=nrow(obs),ncol=1),
    ...
    obj1 <- MakeADFun(data,parameters,random="u",DLL="dfa1tmb_a",silent=T,map=list(Z=Zfac,D=Dfac,cholCorr=cholFac,logsdObs=logsdObsFac,covState=covStateFac))#,map=list())
    ...
    ScaleFac<-as.vector(apply(pl1$u,2,FUN=sd))##as.vector(apply(pl1$u,2,FUN=function(x){max(Mod(x))/3}))#function(x) max(Mod(x))/3))#
    ...
    FitSeries<- pl1$Z %*% pl1$u + pl1$D %*% Covars + pl1$A
    ...
    if(EstSE){
      Zerr.rot<-matrix(sdr$sd[which(names(sdr$value)=='Z')],nrow(pl1$Z)) %*% H.inv
      Uerr.rot<-solve(H.inv) %*% matrix(sdr$sd[which(names(sdr$value)=='u')],nrow(pl1$u)) 
      SES <- list(D=sdr$sd[which(names(sdr$value)=='D')],
                  A=sdr$sd[which(names(sdr$value)=='A')],
                  Z=Zerr.rot*ScaleFac,
                  u=Uerr.rot/ScaleFac,
