...
* USFS data from Luca.
* ADFG data from Heather. Note that she had also archived ~ 1 year of data on KNB, but sent everything over so no longer using that data.
* NPS data from Trey. Note that source/data folder is what was on the KNB archive and has been formatted. But need to bring in new data and combine.
* Native Village of Eyak - 2 sites.
* (in BB repo) NPS data from Krista collected by Dan Young.
* (in BB repo) UW data, streams are formatted, but need QA. 
...
Sites file. This is a longer list of names from akoats that we should probably keep. We may not know all 
