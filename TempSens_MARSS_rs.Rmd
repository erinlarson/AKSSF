---
title: "TempSens_MARSS_rs"
output: html_document
---

Code by T. Cline, edited to run for all years.
...
MetaDat <- readRDS('data_preparation/final_data/md.rds')
...
Tdat <- readRDS('data_preparation/final_data/summer_data_wair2021-11-23.rds') %>% 
...
key difference is here - changed from 2011 to 2000 so running all years of data where we have MODIS snow metrics.
...
TempSens_MARSS <- lapply(2000:2020,FUN=function(y){
  #y<-2011
...
TempSens_ARIMA <- lapply(2000:2020,FUN=function(y){
  #y<-2011
  MeanDWT_TS <- Tdat %>% filter(Year==y) %>% xtabs(meanDT ~ SiteID + DOY,data=.)
  MeanDWT_TS[MeanDWT_TS==0] <- NA #xtabs puts zeros in for missing dates in the dataset, we set those exact zeros to NA
...
saveRDS(TempSens_ARIMA,file=paste0('output/TempSens_Arima_Results_from2000_',Sys.Date(),'.RDS'))
