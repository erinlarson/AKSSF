...
# load cik.daily
# cik.daily <- read_csv("data_preparation/final_data/Daily_Data/cikDaily_Data2021-03-12.csv")
# cik.md <- read_csv("data_preparation/final_data/Metadata/cikMetadata2021-03-11.csv")
# 
# cik.daily <- left_join(cik.daily, cik.md %>% select(SiteID, Waterbody_name))

daily <- read_csv("data_preparation/final_data/zenodo1/dailyTemps.csv")
md <- read_csv("data_preparation/final_data/zenodo1/tempSitesMetadata.csv")
...
cik.daily %>% distinct(SiteID)

#save Deshka for Little Su analysis
# saveRDS(cik.daily %>% filter(SiteID == "CIK_28"), file = "W:/Github/Little_Susitna/data/deshka_2008-2018.rds")

```

# Temperature Figures

Create data frames of mean daily and mean monthly temperatures. Use functions that calculate the mode of the time difference for each day and the number of days in a month. For screening 90% of measurements in a day and 90% of days in a month.
...
ltMonths <- cik.monthly %>% 
  ungroup() %>% 
  distinct(SiteID, Waterbody_name, year, month) %>% 
  count(SiteID, Waterbody_name, month) %>% 
  filter(n > 8)

monthlyLMs <- cik.monthly %>% 
  right_join(ltMonths) %>% 
  nest(data = c(-SiteID, -Waterbody_name, -month)) %>% 
  mutate(
...
monthlyLMs2 %>% 
  filter(p.value < 0.05, term == "year") %>% 
  ggplot(aes(x = as.factor(month), y = estimate * 10, color = Waterbody_name)) +
  geom_point(size = 2) +
  theme_minimal() +
  geom_abline(aes(intercept = 0, slope = 0)) +
...
monthlyLMs2 %>% 
  filter(Waterbody_name == "Deep Creek")
```
...
junelm <- left_join(ltMonths, cik.monthly) %>% filter(month == 6) %>% 
  lme(meanMon ~ year, data = ., random = ~1|SiteID) 
hist(resid(junelm))
summary(junelm)

julylm <- left_join(ltMonths, cik.monthly) %>% filter(month == 7) %>% 
  lme(meanMon ~ year, data = ., random = ~1|SiteID) 
hist(resid(julylm))
summary(julylm)

auglm <- left_join(ltMonths, cik.monthly) %>% filter(month == 8) %>% 
  lme(meanMon ~ year, data = ., random = ~1|SiteID) 
hist(resid(auglm))
summary(auglm)
...
# Linear Trends in Summer Temperatures

Compare to what we had for the Chinook manuscript and we only measured ~0.2 degrees per decade an order of magnitude lower.

```{r}
#334 total to 279

sumTS <- cik.daily %>% 
  group_by(SiteID, Waterbody_name, year = year(sampleDate)) %>% 
  mutate(sumCt = sum(month(sampleDate) %in% 6:8)) %>% 
  distinct(SiteID, Waterbody_name, year, sumCt) %>% 
  filter(sumCt > 0.8*92)  
```

```{r}
cik.summer <- left_join(sumTS, cik.daily) %>%
  filter(month(sampleDate) %in% 6:8) %>% 
  group_by(SiteID, Waterbody_name, year = year(sampleDate)) %>%
  summarize(meanSum = mean(meanDT))


```

```{r}
summerLMs <- cik.summer %>% 
  filter(year < 2019) %>% 
  mutate(yrct = n()) #%>% 
  filter(yrct > 8) %>% 
  nest(data = c(-SiteID, -Waterbody_name)) %>% 
  mutate(
    fit = map(data, ~ lm(meanSum ~ year, data = .x)),
    tidied = map(fit, tidy)
  ) %>% 
  unnest(tidied)
...
For Little Susitna site tour, look at rates of change for temperatures using same metric used to discuss thermal requirements. 
7DADM = Annual maximum of 7-day rolling average of daily maximums.

```{r}
cik.7dayradm <- cik.daily %>% 
  # filter(!year == 2019) %>% 
  group_by(SiteID, Waterbody_name, year = year(sampleDate)) %>% 
  mutate(count = sum(month(sampleDate) %in% 6:8)) %>% 
  filter(count > 0.8*92) %>% 
  arrange(SiteID, sampleDate) %>% 
  mutate(max7d = rollmean(maxDT, k = 7, fill = NA, align = "center")) 
```

```{r}
cik.7dadm <- cik.7dayradm %>% 
  group_by(SiteID, Waterbody_name, year) %>% 
  summarize(annmax = max(max7d, na.rm = TRUE))
...
```{r}
sevendadmLMs <- cik.7dadm %>% 
  filter(year < 2019) %>% 
  mutate(yrct = n()) %>% 
  filter(yrct > 8) %>% 
  nest(data = c(-SiteID, -Waterbody_name)) %>% 
  mutate(
    fit = map(data, ~ lm(annmax ~ year, data = .x)),
    tidied = map(fit, tidy)
  ) %>% 
  unnest(tidied)
