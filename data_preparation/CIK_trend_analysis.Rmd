...
```{r}
cik_dat <- cik_dat %>% temp_msmt_freq()

cik_daily <- cik_dat %>% 
  filter(UseData == 1) %>% 
  group_by(SiteID, Waterbody_name, year, sampleDate, mode_diff) %>%
...
cik_monthly <- cik_daily %>% 
  ...
  group_by(SiteID, Waterbody_name, year, month = month(sampleDate), daysMonth) %>%
...
for(i in 1:nrow(ltMonths)) {
  dat <- cik_monthly %>% right_join(ltMonths %>% slice(i)) %>% pull(meanMon)
...
```{r}
all <- cik_monthly %>% right_join(ltMonths) %>% ungroup() %>% expand(SiteID, year, 6:9) %>% rename(month = `6:9`)
