...
def costdist(riv_in, cat_in, slp_in, costpath, outp, asp_in, dem):
    # Start time
    ...
    cell = 10
    arcpy.PolylineToRaster_conversion(riv_in, oid_fieldname, os.path.join(outp, "riv_raster"), "MAXIMUM_LENGTH", "NONE",
                                      cell)
    ...
    with arcpy.da.SearchCursor(os.path.join(outp, "cat_riv"), ["OID@", "SHAPE@"]) as cursor:
        for row in cursor:
            ...
            # #Try Esri Recommended DistanceAccumulation() instead of CostDistance()
            # # River Raster as source
            # insource = arcpy.Raster(os.path.join(outp, "riv_raster"))
            # incost = arcpy.Raster(slp_in)
            # in_hor = arcpy.Raster(asp_in)
            # insurf = arcpy.Raster(dem)
            # cost_dist = DistanceAccumulation() # Test in arcgis and examine outputs before implementing
