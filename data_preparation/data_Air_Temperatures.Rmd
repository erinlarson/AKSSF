...
md <- readRDS("final_data/md.rds")

write.csv(md %>% distinct(SiteID, Latitude, Longitude), file = paste0("sites", Sys.Date(), ".csv"),
          row.names = FALSE)
```
...
dm_batch <- download_daymet_batch(file_location = "sites2021-11-23.csv",
                      start = 1980,
                      end = 2020,
                      simplify = TRUE,
                      silent = TRUE)
...
write_csv(dm_batch, "daymet/site_daymet.csv")
