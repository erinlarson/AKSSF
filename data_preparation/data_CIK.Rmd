...
cik.md <- cik.md %>% 
  select(SiteID, AKOATS_ID = seq_id, SourceName:Longitude, Sensor_Placement, Waterbody_name, Waterbody_type,
         Sensor_accuracy, Sensor_QAQC) 
...
write.csv(cik.data, paste0("data_preparation/final_data/CIK_AKTemp_", 
                           as.character(date.now),".csv"), row.names = F)

cik.md %>% 
  select() %>% 
  write.csv(paste0("data_preparation/final_data/CIK_Metadata_", 
                           as.character(date.now),".csv"), row.names = F)

cik.daily <- temp_msmt_freq(cik.data)  
cik.daily <- daily_screen(cik.daily %>% mutate(UseData = 1))

#daily.file <- temp_msmt_freq(x) %>% daily_screen(.)

write.csv(cik.daily, paste0("data_preparation/final_data/CIK_Daily_Temp_", 
                            as.character(date.now),".csv"), row.names = F)
