...
cik.data1 <- cik.data1 %>% 
  ...
  select(SiteID, sampleDate, sampleTime, DT, Temperature = `...3`)
```
...
Just keep the cook inlet sites, Sue provided more sites and years of data in the data request specifically for Bristol Bay and those data are already formatted there.
...
Make sure that SiteIDs in the data link to AKOATs and get the waterbody name so that we have it for plotting. All Cook Inlet sites are in akoats. 
...
For duplicates, it looks like a few datasets were provided twice and one should be removed before reading in. I see the two duplicate files for 2018, which were provided in separate folders by Sue for the newest data (post-2017). These are now removed above so not being read in twice. I can't find the duplicate data for CIK 14 in 2011, but the values are exactly the same so averaging won't change anything.

Mean and sd of differences between duplicate values by site. 

I can't find the duplicate reading for CIK_8 on 2009-05-17. The 8.295 reading is accurate based on the file from May 2009 for that site so remove the duplicate reading of 5.670. All of the other differences are pretty minor so can be averaged.
...
```{r}
cik.data %>% ungroup() %>% count(UseData)
...
Save raw data for AKTEMP, which includes date and time. 

```{r}
cik.data %>% 
  mutate(UseData = 1) %>% 
  saveRDS("data_preparation/formatted_data/cik.data.rds")
```

Save metadata file, which includes extras like lat/long and waterbody name.

```{r}

```

Save daily min, mean, and max for actual analysis. These daily summaries can be used to generate temperature metrics and also the thermal sensitivity. Use function to screen out days with < 90% of measurements.

```{r}
source("day_and_month_functions.R")
cik.daily <- temp_msmt_freq(cik.data)

cik.daily %>% 
  ungroup() %>% 
  distinct(SiteID, mode_diff)

cik.daily <- daily_screen(cik.daily %>% mutate(UseData = 1))

cik.daily %>% ungroup() %>% distinct(mode_diff, msmtCt)
cik.daily %>% ungroup() %>% filter(is.na(mode_diff))

cik.daily %>% 
  select(SiteID, sampleDate, meanDT, maxDT, minDT) %>% 
  saveRDS("data_preparation/final_data/cik.daily.rds")
