...
save_aktemp_files(deshka_temp, "anchor")

save_daily_files(deshka_daily, "anchor")
```
...
site_loc %>% select(Description, siteID) %>% 
  left_join(akoats, by = c("Description" = "Agency_ID"))


benM <- akoats %>% filter(Contact_person == "Benjamin Meyer") %>% 
  select(Agency_ID, SourceName, Contact_person, Contact_email, Contact_telephone, Sensor_Placement)

kenai_md <- kenai_daily %>% 
  distinct(SiteID) %>% 
  left_join(kenai_sites) %>% 
  mutate(seq_id = NA,
         SourceName = case_when(grepl("KWF1|KWF2|KWF3", SiteID) ~ "KWF",
                                TRUE ~ "UAF"),
