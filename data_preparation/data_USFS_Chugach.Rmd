...
center_cr <- read_csv(fs_files[grepl("CenterCrk", fs_files)]) %>% 
...
bench_cr <- read_excel(fs_files[grepl("BENCH", fs_files)]) %>% 
  rename(Date = "Date",
         Time = "Time",
...
daves_cr <- read_excel(fs_files[grepl("DavesCrk_surfaceH2O_cz_2018", fs_files)]) %>% 
  rename(Date = "Date Time,",
         Time = "GMT-08:00",
...
quartz_cr <- read_excel(fs_files[grepl("QuartzCrk_surfaceH2O_cz_2018", fs_files)]) %>% 
  rename(Date = "Date Time,",
         Time = "GMT-09:00",
...
fsdat <- kendat %>% rename(Agency_ID = SiteID,
                           sampleTime = Time,
                           sampleDate = Date) %>% 
  bind_rows(fsdat)
```
...
fsdat <- fsdat %>% 
  mutate(useSite = case_when(Agency_ID %in% c("Clear Creek", "Hatchery Creek", "Ibeck Creek", "Jack Bay River",
                                           ...
                                           "Rude River SC", "SF Williwaw Creek", "Solf Lake Inlet", 
                                           "Steller Jay Creek") ~ 0,
                             TRUE ~ 1),
