...
Correlation between day length and air temperatures.

```{r daylength-air temp plots}

air_dat <- readRDS("data_preparation/final_data/summer_data_wair_dayl2022-03-01.rds")

air_dat %>% 
  summarize(cor(airDT, daylen_min),
            cor(airDT, global_dayl))

air_dat %>% 
  ggplot(aes(x = daylen_min, y = airDT)) +
  geom_point() +
  stat_cor() +
  stat_regline_equation(aes(label =  paste(..eq.label.., ..adj.rr.label.., sep = "~~~~")),
                        label.x = 650, label.y = 21) +
  labs(title = "Site-specific daylength")


air_dat %>% 
  ggplot(aes(x = global_dayl, y = airDT)) +
  geom_point() +
  stat_cor() +
  stat_regline_equation(aes(label =  paste(..eq.label.., ..adj.rr.label.., sep = "~~~~")),
                        label.x = 650, label.y = 21) +
  labs(title = "Global daylength")
