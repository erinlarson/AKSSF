...
mod_dat <- readRDS("data_preparation/final_data/model_data2022-05-09.rds")

sp_dat <- mod_dat %>% 
  select(Region, SiteID = Site, Year, str_ord:log_precip) %>%
  distinct() %>% 
  left_join(md %>% select(SiteID = Site, Waterbody_name)) %>% 
  mutate(SiteID = tolower(SiteID))

#check that all sites in Tim's output have spatial data. Note that he dropped a lot of sites so use a left join.
left_join(TempSens_Interaction_Subset1 %>%
            filter(!Year == 2020) %>% 
            distinct(SiteID) %>% 
            mutate(SiteID = tolower(SiteID)),
          md %>% 
            mutate(SiteID = tolower(Site))) %>% 
  filter(is.na(Latitude)) #good looks like everything is matching up.
