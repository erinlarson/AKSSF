...
* For daymet, air temperatures were extracted by site location. Daymet are on a 1 km grid. They have not been processed by catchment because the daymet netcdf subset service was down from mid-May until June 10 when I was working on this. I further investigated the sizes of our catchments and realized they are mostly < 1 km^2^ so it doesn't make sense to aggregate at that spatial scale.
...
sumdat <- readRDS("data_preparation/final_data/archive/summer_data_wair2021-07-30.rds")
md <- read_rds("data_preparation/final_data/archive/md.rds")
...
# Catchment air temperatures

Note: the previous comparison between catchment and site air temperatures was already smoothed over a moving window so it wasn't an accurate comparison. I re-extracted catchment temperatures for three catchments in the Anchor and they came out the same as site air temperatures because the catchments were smaller than 1 km^2^, the daymet cell size.

Catchments are quite small in both the NHDPlus and our synthetic networks so there isn't a good reason to take an average over the catchment.

```{r get catchments for Cook Inlet, eval = FALSE}
...
st_area(cats_ci) %>% 
  ...
  summary() %>% 
  kable(caption = "Summary of catchment sizes for Cook Inlet NHDPlus")
```

Other options would be to summarize by watershed, but I don't think there would a good explanatory power between air temperatures over large watersheds and stream temperatures. Or, a moving window (e.g. 3x3 cells). The downside to that approach would be extracting air temperatures from cells that don't drain to the site locations.
...
Look at number of sites by region and year using 8/31 end date and 80% completeness as the data filter. (This was discussed during a meeting on 8/17/21.)

```{r}
sumdat3 <- jd_completeness_filter(sumdat, 243, 80)

sumdat3 <- left_join(sumdat3, md %>% select(SiteID, Region))
...
sumdat3 %>% 
  ...
  labs(x = "Year", y = "Number of Sites", title = "Sites with 80% of days in June 1 - August 31 window") +
  theme_bw() +
...
# Differences in maximum water temperatures 

```{r}
sumdat3 %>% 
  count(Region, SiteID, year) %>% 
  nrow()

sumdat3 %>% distinct(Region, SiteID) %>% count(Region)

sumdat3 %>% 
  group_by(Region, SiteID, year) %>% 
  summarize(max_daily = max(meanDT)) %>% 
  ggplot() +
  geom_boxplot(aes(x = Region, y = max_daily)) +
  geom_jitter(data = . %>% filter(!year == 2019), aes(x = Region, y = max_daily), color = "grey", size = 0.7) +
  geom_jitter(data = . %>% filter(year == 2019), aes(x = Region, y = max_daily), color = "red", size = 0.7) +
  # geom_jitter(data = . %>% filter(SiteID %in% c("CIK_28", "Deshka 6 Downstream", "Deshka 6 Upstream")), 
  #             aes(x = Region, y = max_daily), shape = 2) +
  theme_bw() +
  labs(y = "Maximum Summer Mean Daily Temperature (°C)")

ggsave("output/boxplot of max temps by region.jpeg")
```
